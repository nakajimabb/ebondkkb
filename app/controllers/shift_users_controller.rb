class ShiftUsersController < ApplicationController
  def shift_table
    if params[:start_date] && params[:end_date] && params[:job_type]
      ...
      @shift_users[:weekly].each{ |wday, shift_users| @shift_users[:weekly][wday] = shift_users.eager_load(:dest) }
      @shift_users[:holiday] = ShiftUser.shift_users_holiday(@span, user_ids).eager_load(:dest)
      @shift_users[:custom] = ShiftUser.where(proc_type: :custom, user_id: user_ids, dated_on: @span).eager_load(:dest)
      @shift_users[:rest_week] = ShiftUser.where(proc_type: :rest_week, user_id: user_ids, dated_on: @span).eager_load(:dest)
      @shift_users[:daily] = ShiftUser.where(proc_type: :daily, user_id: user_ids, dated_on: @span).eager_load(:dest)
